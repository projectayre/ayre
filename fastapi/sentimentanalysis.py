    ...
    def analyze_image_sentiments(self, image):
        ...
        try:
            ...
            if emotions:
                ...
                # get all the emotions with non-zero intensity
                emotions = { k: v for k, v in emotions.items() if v > 0.0 }
                for emotion, intensity in emotions.items():
                    for intensity_label, intensity_range in intensity_map.items():
                        if intensity >= intensity_range[0] and intensity < intensity_range[1]:
                            emotions[emotion] = intensity_label
                            break
                return self.return_string_results(emotions)
            else:
